...
impl CleaningAssignment {
    ...
    pub fn has_overlap(&self) -> bool {
        !(self.0.end <= self.1.start || self.1.end <= self.0.start)
    }
}
...
fn main() {
    let with_overlap = BufReader::new(File::open("input").unwrap())
        .lines()
        .map(|line| CleaningAssignment::from_str(&line.unwrap()).unwrap())
        .filter(|assignment| assignment.has_overlap())
        .count();

    println!("There are {with_overlap} assignments that overlap");
}
